...
def viewwindow():
    ...
    Message(gui,font=("Arial Black", 22, "bold"),text = "NAME      USERNAME      PASSWORD",width=700).pack()
...
def register():
    ...
    if c==d and c!="" and len(c)>5 and a!="" and b!="":
        ...
        if(a=="" or b=="" or c=="" or d==""):
            messagebox.showerror('Oops! something wrong', 'Field should not be empty')
        else:
            messagebox.showwarning('Oops! something wrong', 'Both passwords should be same! \nPassword should contain atleast 6 characters')
    e3.delete(0,END)
    e4.delete(0,END)
    e5.delete(0,END)
    e6.delete(0,END)
...
def login():
    ...
    if (checkuser(a,b))!=None:
        ...
        e1.delete(0,END)
        e2.delete(0,END)
        messagebox.showerror('Oops! something wrong', 'Invalid credentials')
...
def appwindow():
    ...
    def insertitems():
        ...
        if a=="" or b=="" or c=="":
            ...
            messagebox.showerror("Oops! something wrong","DATE should be in format dd-mm-yyyy")
        elif (d.isdigit()==False):
            messagebox.showerro("Oops! something wrong","Cost should be a number")
        else:
            insert(a,b,c)
            e1.delete(0,END)
            e2.delete(0,END)
    ...
    gui = Tk()
    gui.title("EXPENSES")
    gui.iconbitmap("moneyicon.ico")
    gui.configure(bg='#FFB266')
    gui.geometry("900x700")
    l8=Label(gui,width=60,height=7,font=("century",35),bg="#00FF80",text="").place(x=450,y=60)
    l7=Label(gui,width=100,height=10,font=("century",35),bg="#66B2FF",text="").place(x=-455,y=410)
    l1=Label(gui,font=("comic sans ms",17),bg="#FFB266",text="Product name").place(x=10,y=150)
    exp_itemname=StringVar()
    e1=Entry(gui,font=("adobe clean",15),textvariable=exp_itemname)
    e1.place(x=220,y=155,height=27,width=165)
    l2=Label(gui,font=("comic sans ms",17),bg="#FFB266",text="Date(dd-mm-yyyy)").place(x=10,y=200)
    exp_date=StringVar()
    e2=Entry(gui,font=("adobe clean",15),textvariable=exp_date)
    e2.place(x=220,y=205,height=27,width=165)
    l3=Label(gui,font=("comic sans ms",17),bg="#FFB266",text="Cost of product").place(x=10,y=250)
    exp_cost=StringVar()
    ...
    b7=Button(gui,text="Close app",font=("georgia",17),activebackground="#fffa66",activeforeground="red",width=10,command=close_app).place(x=700,y=650)
    l6=Label(gui,width=60,font=("century",35),bg="#CCFFCC",fg="#330019",text="EXPENSE  TRACKER").place(x=-450,y=0)
...
frame = Frame(bd=0, highlightthickness=0, background="#FFA500").place(relx=0.46, y=0, relwidth=5, relheight=1)
frame2 = Frame(bd=0, highlightthickness=0, background="#F4A460").place(x=0, rely=0.78, relwidth=1, relheight=1)
root.title("LOGIN / REGISTER")
root.geometry("1000x680")
l1=Label(root,font=("comic sans ms",19),bg="#F0E68C",text="Username").place(x=80,y=230)
l2=Label(root,font=("comic sans ms",19),bg="#F0E68C",text="Password").place(x=80,y=280)
...
l6=Label(root,font=("comic sans ms",19),bg="#FFA500",text="Name").place(x=653,y=195)
l3=Label(root,font=("comic sans ms",19),bg="#FFA500",text="Username").place(x=604,y=243)
l4=Label(root,font=("comic sans ms",19),bg="#FFA500",text="Password").place(x=610,y=293)
l5=Label(root,font=("comic sans ms",17),bg="#FFA500",text="Confirm password").place(x=532,y=342)
...
Label(root,width=60,font=("consolas",35),bg="black",fg="yellow",text="EXPENSE MANAGER").place(x=-300,y=0)
b3=Button(root,text="Exit Window",font=("impact",15),activebackground="#fffa66",activeforeground="red",width=10,command=root.destroy).place(x=410,y=630)
b4=Button(root,text="Delete all users",font=("impact",15),activebackground="#fffa66",activeforeground="red",width=12,command=deleteallusers).place(x=130,y=620)
b5=Button(root,text="View all users",font=("impact",15),activebackground="orange",activeforeground="blue",width=12,command=viewwindow).place(x=130,y=560)
root.resizable(False, False)#disable minimize/maximize button
root.mainloop()